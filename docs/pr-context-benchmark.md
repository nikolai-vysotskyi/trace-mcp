...
- **The truncation failure mode fires on most of this dataset.** The trace-mcp
  ...
  changed symbols arrive without their bodies, 98 of 338 across the corpus.
...
- **The coverage gap is what buys the saving.** Of the 98 changed symbols that
  arrive bodyless, **93 are budget truncation**. Re-running the same corpus at
  larger bundle budgets trades one for the other, monotonically: 8,000 → 240 of
  338 bodies at {{ site.data.pr_context_bench.median_savings_pct }}% saved;
  16,000 → 263 at 32.3%; 32,000 → 299 at **−0.1%**; 64,000 → 333 at **−0.3%**.
  The saving crosses zero while 39 bodies are still missing, so on this dataset
  there is no budget that buys full changed-symbol coverage *and* a token win.
  Five bodies never arrive at any budget; those are a defect, not a tradeoff,
  and are open. Per-symbol counts:
