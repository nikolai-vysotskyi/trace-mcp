...
It also produces a **worse review**. Sending both contexts to the same model on
the same PRs, the trace-mcp arm understood the change
{{ site.data.pr_context_quality.understood_delta_pp }} percentage points less
often and asserted about twice as many problems that are not there — a
preregistered bar this run [missed]({{ '/perf/prereg-pr-quality/' | relative_url }}).
Both halves are below; neither number should be quoted without the other.
...
measured now, and **the answer went against us**: on the same pull requests,
the review written from trace-mcp's context understood the change
{{ site.data.pr_context_quality.understood_delta_pp }} percentage points less
often than the review written from the full files, and asserted roughly twice
as many problems that are not there. The
...
PR — was written before the calls were made, and this run **missed both**.
...
Latency is the one column trace-mcp wins, which is what a tenth of the input
buys. It does not pay for the other two.

So the honest reading of this benchmark is now two sentences, not one:
**trace-mcp's review context costs about a tenth of the tokens, and at the
current 8,000-token bundle budget it costs review quality too.** Closing that
gap — a larger or better-packed bundle, measured on this same harness — is the
work this result creates, and until it lands the token saving should not be
quoted on its own. Every review and every judgement is committed in
