...
- **Advertised tool-surface cost — narrowed 2.5×, still behind.** Re-measured August 29, 2026: the shipped default is now 54 tools / ~18.8K tokens of `tools/list` plus ~1.75K tokens of server instructions, down from the ~51K this page quoted in August. The preset-bypass bug behind that number is fixed and closed. The remaining gap is real but no longer embarrassing: the two largest peers advertise ~1.9K and ~7K tokens, i.e. ~11× and ~3× cheaper than our default. Both buy that with a smaller *capability* surface (one peer advertises a single tool); we buy ours by dropping 111 tools from the default and keeping them one config line away. What is still ours to fix is that ~57% of the default wire is tool and parameter *descriptions* (22.5 KB of 67.7 KB) — `description_verbosity: "none"` halves the surface to ~8.4K tokens today, but at a cost to tool-selection accuracy nobody has measured. A middle setting that keeps the first sentence and drops the rest is the obvious unexplored win.
