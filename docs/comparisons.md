...
Where it differs is infrastructure and semantics. SocratiCode requires Docker running local Qdrant and Ollama containers (or external API keys), advertises all 25 tools unconditionally without preset filtering, parses syntax without resolving framework connections, and ships under copyleft AGPL-3.0. trace-mcp runs locally with zero external infrastructure (your code and index stay on your machine — see [privacy](/privacy.html))—single `npx trace-mcp` with embedded SQLite+FTS5 and local ONNX embeddings—under permissive MIT, with {{ site.data.counts.languages }} languages, {{ site.data.counts.frameworks }} framework integrations, and active refactoring tools. Full head-to-head: [trace-mcp vs SocratiCode](/vs/socraticode.html).

### trace-mcp vs TokenSave — 86 advertised tools vs framework-aware graph depth
{: #vs-tokensave}
...
### Pass of September 8, 2026 — current

- **First source read of TokenSave** (aovestdipaperino/tokensave, Rust, MIT, v7.11.1, {{ site.data.competitors.tokensave.stars }} stars), from the previous pass's priority queue. Read through the GitHub API at `master`: package metadata, `Cargo.toml`, `server.json`, `src/mcp/tools/definitions.rs`, `src/mcp/server.rs`, `src/tokensave/memory.rs`, `src/tokensave/staleness.rs`, `src/accounting/`, and the extraction worker pool (`src/extraction_worker.rs`). It corrected the table: written in **Rust** (edition 2021), embedded libSQL (SQLite fork) + FTS5 with ONNX Runtime (`ort`) embeddings, **86 MCP tools defined and 85+ advertised on standard MCP hosts** (only 5 carry `anthropic/alwaysLoad: true`), costing ~12-15K tokens in schema overhead per session start; tree-sitter AST parsing across 50+ languages in three feature tiers; subprocess worker crash isolation; bounded catch-up sync ceiling (2,000 files); and string replacement primitives. Head-to-head analysis: [trace-mcp vs TokenSave](/vs/tokensave.html). Findings and the take-or-pass on each mechanism are in the profiling depth tracker at the end of this page.
- **Star re-check against the GitHub API** for all tracked competitors: trace-mcp 166 → 170, jCodeMunch 2.7K (2,668), TokenSave 621, SocratiCode 3.3K (3,287), codegraph 69.9K (69,891), codebase-memory-mcp 42.5K (42,504), Serena 28.9K (28,902), Repomix 28.2K (28,221), Context Mode 20.5K (20,487), code-review-graph 31.2K (31,219), CodeGraphContext 4.2K (4,168), CodeGraph (codegraph-ai) 80.
- Rows for other projects were not re-checked in this pass and carry the September 7 figures.
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **GitHub stars** | {{ site.data.competitors.trace_mcp.stars }} | {{ site.data.competitors.repomix.stars }} | {{ site.data.competitors.context_mode.stars }} | {{ site.data.competitors.code_review_graph.stars }} | {{ site.data.competitors.jcodemunch.stars }} | {{ site.data.competitors.codebase_memory_mcp.stars }} | 165 |
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **GitHub stars** | {{ site.data.competitors.trace_mcp.stars }} | new (2026) | ~56.7K | 91.8K | ~64.3K | ~6.4K | 765 |
...
_**depgraph.ai** (archview-ai, closed source, hosted) is the one peer on this SERP that is not in the table above, because it is not the same product: it indexes **third-party packages, not your repository**. Verified on its own site September 6, 2026 — it publishes one curated remote MCP endpoint per public package (`https://mcp.archcontext.ai/github_<owner>_<repo>_branch_<branch>/mcp`, e.g. `github_shadcn-ui_ui_branch_main`), advertising find / trace / prove over that package's functions, classes and files across 20+ languages. There is nothing to run locally and nothing to point at your own code. Pricing is per user per month with quota measured in MCP-delivered context: Free 20 MB, Pro $15 / 100 MB, Max $75 / 400 MB. Its pricing page links a community repository at `github.com/archview-ai/depgraph-community`; that URL returned 404 on September 6, 2026, and the `archview-ai` account has one unrelated public repo, so there is no source to read and no star count to quote — which is why it is absent from `docs/_data/competitors.yml`, whose every entry must name a checkable repository. **Take:** complementary, not competing. Understanding a dependency you did not write and navigating the code you did write are different questions; a reader comparing the two should know they can hold both._
...
**Profiled deep (architecture/code read, explicit take-or-pass with reasoning):** Graphify, Headroom, Kage, mem0/OpenMemory, MemPalace, codebase-memory-mcp, codegraph, SDL-MCP, Serena, code-review-graph, CodeGraphContext, LeanKG, Roam-Code, Repomix, marm-memory, CodeGraph (codegraph-ai), SocratiCode, jCodeMunch, TokenSave.

**Tracked, still surface-level only (README + stars, no code/architecture read yet):** Narsil-MCP, cymbal, DeepContext, smart-coding-mcp, mcp-local-rag, knowledge-rag, ConPort, engram, claude-mem, repo-context-mcp, grafel, GitNexus, Code Pathfinder.
...
**CodeGraphContext profiled this pass** (September 4, 2026 — read through the GitHub API at `main`, no clone: repository metadata, `pyproject.toml`, `README.md`, `src/codegraphcontext/tool_definitions.py`, `server.py`, `tools/scip_indexer.py`, `tools/indexing/scip_pipeline.py`, `core/bundle_registry.py`). It had been a table row since April on README and stars alone; this pass exists because `/comparisons.html` ranks first for `codegraphcontext` in Search Console on two passing mentions, which is a poor reason for a reader to arrive and find nothing (measured by the SEO Agent during review of PR #842). Facts worth recording rather than re-deriving: the repo moved from `Shashankss1205/CodeGraphContext` to its own `CodeGraphContext` org, 4,155 stars at this reading, v0.6.10 and still self-classified alpha; 29 tools defined and all 29 advertised, trimmable only by an `mcp.json` `disabledTools` denylist; the SCIP path is off by default and needs a per-language Sourcegraph indexer binary, with C/C++ additionally requiring `compile_commands.json`; and its `.cgc` bundle registry is a Hugging Face dataset (`codegraphcontext/registry`), the only shareable pre-built graph distribution anyone in this field ships. The September 4 pass deliberately shipped no `/vs/` spoke — not because the format underperforms, but because none of the six existing spokes has been indexed yet (GSC URL Inspection, September 6, 2026: two "Discovered — currently not indexed", four unknown to Google). **Reversed on September 6, 2026**, and only for this peer: TRA-1022 established the same day that Google's copy of the sitemap was five days stale and one spoke had never been in it at all, so "the spokes are not indexed" was partly a submission fact rather than a demand fact, and the sitemap has since been resubmitted and refetched. This is also the one peer with a measured brand query arriving on the hub, which gives the spoke a landing target the other six were built without. [/vs/codegraphcontext.html](/vs/codegraphcontext.html) re-verifies the facts above at repository head on September 6, 2026 — 0.6.13, eleven SCIP families, {{ site.data.competitors.codegraphcontext.stars }} stars. Structure and internal linking of the spoke remain the SEO autopilot's call; the content is this page's.
...
- **The Counter meta-dispatch front door (`order`, `menu`, `route`): passing on meta-dispatch, validating the prompt budget lesson.** Fronting 90 tools with a single `order(action, args)` dispatch verb drastically cuts top-level schema tokens, but forces an extra indirection turn and bypasses client schema validation, increasing parameter hallucination risk. trace-mcp instead uses task-tailored presets (`minimal` default with 28 tools, `review`, `architecture`, `dev`) and dynamic runtime escalation via `load_tools`, preserving type-safe client schemas without meta-tool indirection. Nothing to take.
- **Agent configuration hygiene auditing (`audit_agent_config`): taking the idea for doc and config linting.** `audit_agent_config` cross-references `CLAUDE.md`, `.cursorrules`, and `copilot-instructions.md` against the symbol index to find dead symbol names, broken paths, and token waste. We already check doc-to-code path integrity (`verify_docs`, `tests/docs/doc-path-refs.test.ts`), and extending this to user-facing agent instruction files is a high-value developer workflow.
- **Text-based refactoring plan generation (`plan_refactoring`): passing, we already do better.** `plan_refactoring` generates text find-and-replace candidate blocks `{old_text, new_text}` using regexes on imports. trace-mcp provides native AST refactoring write tools (`refactor_rename`, `refactor_extract`, `refactor_move`, `refactor_codemod`) that execute atomic, syntax-verified transformations directly in the AST. Nothing to take.

**TokenSave profiled this pass** (September 8, 2026 — read through the GitHub API at `master`, no clone and nothing executed: repository metadata, `Cargo.toml`, `server.json`, `src/mcp/tools/definitions.rs`, `src/mcp/server.rs`, `src/tokensave/memory.rs`, `src/tokensave/staleness.rs`, `src/accounting/`, `src/extraction_worker.rs`). 621 stars (GitHub API, this pass), MIT, Rust (edition 2021), v7.11.1. It defines 86 MCP tools in `get_tool_definitions()` and advertises all 85+ on standard MCP hosts, with only 5 tools carrying `anthropic/alwaysLoad: true` (`tokensave_search`, `tokensave_context`, `tokensave_callees`, `tokensave_impact`, `tokensave_status`). Storage is embedded libSQL (SQLite fork) with FTS5 and local ONNX runtime (`ort`) embeddings. AST extraction covers 50+ languages across three compilation tiers (lite 11, medium +9, full +40 more) with zero framework awareness. See the full head-to-head: [trace-mcp vs TokenSave](/vs/tokensave.html).

Four mechanisms were read at source and each got a decision:

- **Subprocess-isolated extraction worker pool (`extraction_worker.rs`): noting the architecture, our runtime is already safer.** To prevent crashes (segfaults, aborts) in native C/Rust tree-sitter grammars from killing the MCP server daemon, TokenSave routes file extraction through a pool of isolated worker child processes that can be respawned if killed. trace-mcp instead executes tree-sitter parsers inside a WebAssembly sandbox (`web-tree-sitter`), where memory safety and process isolation are enforced by the V8 WASM runtime — eliminating native segfaults and panics by design without subprocess IPC overhead. Nothing to take.
- **Bounded catch-up sync ceiling (`AutoSyncScope::TooManyStale`): taking the guard concept.** Automatic background syncs on tool calls or server start cap sync volume at 2,000 stale files, refusing to turn a background access check into a runaway initial indexing pass over multi-terabyte uninitialized trees. We already track dirty files incrementally, but formalizing an explicit ceiling on automatic background syncs is a sound defensive pattern.
- **Multi-branch isolated index databases: passing, unified index is more efficient.** TokenSave allows opting into separate libSQL database files per git branch (`.tokensave/branches/<branch>.db`), with a branch drift guard (`BranchDrift`) stopping cross-branch writes. trace-mcp tracks branch and commit SHAs directly in unified SQLite WAL tables, allowing instant branch switches and cross-branch diff queries without managing fragmented database files. Nothing to take.
- **String replacement primitives (`tokensave_str_replace`): passing, we already do better.** TokenSave provides text-slice find-and-replace tools with anchor matching. trace-mcp provides verified AST refactoring tools (`refactor_rename`, `refactor_extract`, `refactor_move`, `refactor_codemod`) that execute atomic, syntax-checked code transformations with cross-file import graph rewrites. Nothing to take.

Priority for next deep-dive: **Narsil-MCP** (~100 stars, 32 languages, 90 tools, multi-engine AST graph), the next direct code-graph peer remaining unprofiled at source.
