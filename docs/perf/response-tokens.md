...
- **Measured calls only.** `savings.json` gained a `measured` block that is
  ...
  While the harness benchmark table above uses exact `o200k_base` counts, the
  runtime per-install tracker converts measured response wire characters using
  a zero-overhead `chars / 4` estimate (~0.250 tokens/char, reflecting the
  0.220–0.368 empirical spread measured across the 25 busiest tools).
