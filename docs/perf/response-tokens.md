...
response text, not an estimate. `search` is the exception: it is the **mean over
a fifteen-query basket**, median across runs, because one query was deciding a
quarter of the aggregate — see below. Call volume is this machine's
...
| `get_call_graph` | 14 | 1,500 | **1,633** | 1.09 |
...
1. **0.15 is wrong on every tool that matters.** The four busiest (88% of all
   calls) measure 0.33–1.44. The assumption is off by 2.2x on the best of them.
...
| | TRA-880 (12 tools) | TRA-945 (24 tools) | TRA-952 (shaped) | TRA-985 (basket) |
|---|---|---|---|---|
...
| `search` measured over | 1 query | 1 query | 1 query | **15 queries** |
...
The last column is not comparable to the three before it on the `search` row and
therefore not on the aggregate either: TRA-985 changed how `search` is sampled,
which is the point of that column. On the old sampling the same commit reads
30.1%. Both numbers are stated in that section rather than the more flattering
one being picked.
...
## One query was deciding a quarter of the aggregate (TRA-985)

`search` is 4 441 of 18 319 recorded calls — 24% of the weight in the table
above — and until this run its published row was **one query**, `savings`,
against this repo. Measured over a fifteen-query basket instead, the same build
that published 924 tokens per call averages **1 417**. The bench had been
quoting the cheap end of a 3.6x spread as if it were the tool's cost.

That matters more than the size of the error, because of what the error was
about to buy. The response-shaping fix below, measured the old way, takes the
aggregate from 21.1% to **30.1%** — clearing the declared 25% bar for the first
time. Measured over the basket, the same code change takes it from 13.2% to
**{{ site.data.response_tokens.reduction_pct }}%**, and the bar is missed again.
Same commit, same tools, same call weights; the protocol decides the verdict.
The basket is what is published, and the pass is not claimed.

The basket is now in the harness (`SEARCH_BASKET` in
`scripts/bench-response-tokens.ts`), so every future run measures `search` the
same way. Its fifteen terms are the most common subsystem nouns in this repo's
own directory names, fixed before any of them was measured. Grouped calls
collapse to their **mean**, not their median, because the aggregate multiplies
that number by a call count.
...
| published row, basket mean | 1,417 | **864** |
| ratio against its 600-token baseline | 2.36x | **1.44x** |
...
`search` still costs 1.44x the baseline it is credited against, so it is still
