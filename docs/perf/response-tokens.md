...
| | TRA-880 (12 tools) | TRA-945 (24 tools) | TRA-952 (shaped) | TRA-993 (registered frame) |
|---|---|---|---|---|
...
**The last column is not an improvement on the one before it.** Nothing in the
product changed between them that would move a headline 46 points. What changed
is that `search_text`, `get_outline` and `search` — 76% of the weight — stopped
being priced from one sample each and started being priced from a
...
Read across that row as four *frames*, not four builds.
