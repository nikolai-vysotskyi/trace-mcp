...
Checkouts that share a git remote share one index database only while they sit on the
same commit. A checkout on a different commit registers onto its own database instead,
so its branch-only files never land in the canonical checkout's index. A checkout that
...
> `get_index_health` reports `embedding: { enabled, totalSymbols, embedded,
> coveragePct, queued, pausedUntil, lastError }` and adds a warning — semantic
> and hybrid search results are incomplete until the provider is reachable.
> `embed_repo` ignores the pause and retries immediately.
>
> Vector coverage is reported whenever symbols exist — not only while a pause
> is open (TRA-1904). `ai.enabled` defaults to `false`, so a project that
> never configured AI reports `enabled: false, coveragePct: 0` with an
> FTS-only warning instead of an absent block; an undrained backlog with no
> recorded failure is reported as `degraded` with a backfill hint.
...
| Codex | `~/.codex/config.toml`, `<project>/.codex/config.toml` | TOML | `[mcp_servers.trace]` | A legacy `[mcp_servers.trace-mcp]` table is detected and renamed on `init`. Restart Codex after `init`/`update` — it reads `config.toml` once at startup. See [Verifying the Codex connection](#verifying-the-codex-connection) |
...
| **Antigravity** (Google) | `~/.gemini/config/mcp_config.json` | JSON | `mcpServers` | Global-only (no documented per-project config as of mid-2026). Lives next to the deprecated Gemini CLI's `~/.gemini/settings.json`, which trace no longer reads or writes — the two do not share entries |
...
Codex reads `config.toml` once at startup, so **restart it after every
`init` or `clients update`** — otherwise the entry sits on disk while the
running session still uses the old config. Then verify from either side:

- Inside Codex, run `/mcp` — the `trace` server should list as connected.
- From a shell, run `codex mcp list` — same check without opening a session.
- From this repo, run `trace-mcp clients status --client codex` (add
  `--scope project` for the `<project>/.codex/config.toml` layer) — anything
  but `up_to_date` names the drifted field in `staleReason`, and
  `trace-mcp clients update codex` rewrites it.
