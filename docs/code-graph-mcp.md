...
2. **Measured context reduction.** In real benchmarks, graph-assembled context reduces input token usage by a median **{{ site.data.pr_context_bench.median_savings_pct }}%** to assemble pull-request review context across {{ site.data.pr_context_bench.pr_count }} open-source pull requests in {{ site.data.pr_context_bench.repo_count }} repositories ([read the PR context benchmark](/pr-context-benchmark.html), or explore [7 tactics to cut Claude Code token usage](/reduce-claude-code-token-usage.html)). Blind evaluation confirms comprehension at parity: {{ site.data.pr_context_quality.trace_understood }} understood rate against naive file loading's {{ site.data.pr_context_quality.baseline_understood }} (at {{ site.data.pr_context_quality.trace_false_positives }} vs {{ site.data.pr_context_quality.baseline_false_positives }} false positives per PR; see [quality preregistration](/perf/prereg-pr-quality/)).
...
* **Zero-overhead local storage:** All graph nodes, edges, and FTS5 search indices live in an embedded SQLite database on your machine. For how resident memory and caches are capped, see [daemon memory](/daemon-memory.html). No hosted services, no background daemons requiring cloud accounts, and your source code never leaves your machine ([local privacy guarantee](/privacy.html)).
...
| **Runtime buffer truncation** | [Context Mode](https://github.com/pashpashpash/context-mode) | Intercepts and truncates CLI execution buffers before prompt injection. | Focuses on runtime stdout/stderr pruning; maintains no persistent semantic code graph. [trace-mcp vs Context Mode](/vs/context-mode.html) |
| **Knowledge graph Cypher** | [GitNexus](https://github.com/abhigyanpatwari/GitNexus) | Constructs code knowledge graph in embedded Kuzu graph DB querying via raw Cypher strings. | Non-commercial PolyForm license, requires local C++ toolchain build, and exposes unvalidated Cypher queries. [trace-mcp vs GitNexus](/vs/gitnexus.html) |
| **LLM concept summaries** | [Graft](https://github.com/trailhq/Graft) | Generates plain-language subsystem and concept explanations in markdown files via LLM. | Requires external LLM API keys and model calls for all node generation; no relational graph traversal. |
| **RDF multi-layer CCG** | [Narsil-MCP](https://github.com/postrv/narsil-mcp) | Emits 4-layer progressive disclosure Code Context Graph (CCG) datasets in RDF/N-Quads. | Advertises ~90 tools without role presets; complex semantic web RDF formats rather than lightweight JSON. |
| **Packing vs indexing** | [Repomix vs codegraph](/vs/repomix-vs-codegraph.html) | Side-by-side comparison of full-file prompt packing against single-tool graph exploration. | Evaluates static context serialization against interactive graph queries. [Repomix vs codegraph](/vs/repomix-vs-codegraph.html) |
