...
| Call graph resolution | 5-tier resolution (`compiler_verified` to `fuzzy`) | Word-token matching (`_word_match` regex + import map) |
...
### 4. Call Graph Precision: Multi-Tier Confidence vs. Word-Token Matching

Understanding who calls a function is essential for safe code edits.

In jCodeMunch, static call graph resolution (`_call_graph.py`) relies on text heuristics:
1. It identifies files that import the target symbol's file.
2. It slices the source body of symbols in those files and checks whether the target symbol's name appears as a word token using `_word_match` (`\b<name>\b` regex).
3. In reverse, callees are found by slicing the caller's body and matching words against symbols exported by imported files.

While fast, word-token matching lacks type resolution and cannot disambiguate common function names (e.g. `validate`, `execute`, `init`, `format`) shared across multiple modules.

trace-mcp maintains an explicit, directed edge graph in SQLite with **5 resolution tiers**:
...
AI agents receiving call graph responses from trace-mcp know exactly how confident each relationship is, avoiding dangerous assumptions during refactoring.
...
- **High-confidence impact analysis**: You need a 5-tier call graph that distinguishes compiler-verified relationships from fuzzy token matches.
