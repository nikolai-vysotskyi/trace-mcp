    ...
    },
    {
      ...
      "mainEntity": [
        ...
        },
        {
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "SocratiCode advertises all 25 tools by default (~5.2K tokens). trace-mcp advertises 28 tools on its default minimal preset (~11.6K tokens including server instructions), but allows agents to trim down to a 5-tool minimal preset (~2.4K tokens) or role-specific presets (review, architecture, dev), keeping unadvertised tools reachable via load_tools."
          }
        },
        {
          "@type": "Question",
...
Verified on September 7, 2026 against SocratiCode's repository at `main` (v1.13.0, commit `fdcb17d`, {{ site.data.competitors.socraticode.stars }} stars). Tool definitions from `src/index.ts` and `src/tools/`, storage architecture from `src/constants.ts` and `src/config.ts`, language mappings from `src/constants.ts`.
...
- For ultra-lean contexts, select the 5-tool minimal preset (`minimal`, ~2.4K tokens).
- For focused tasks, choose task-tailored presets: `review` (PR analysis), `architecture` (system boundaries), or `dev` (implementation).
- Any tool not in the current preset remains accessible dynamically via `load_tools` without restarting the server.
