...
Measured at trace-mcp **{{ site.data.response_tokens.measured_build.version }} (`{{ site.data.response_tokens.measured_build.commit }}`)** on {{ site.data.response_tokens.measured_at | date: "%-d %B %Y" }}{% if site.data.measurements.response_tokens.historical %} — a result from that build, not a claim about the current one{% endif %}. Its [preregistration](/perf/prereg-response-tokens/) publishes it as a **pass** — the first, after three misses. The bar was declared unadjustable before the data and has not been moved. Read what cleared it before quoting it: the three busiest tools, 76% of weighted calls, had each been published from **one sample** — one query, one file, one query — and `get_outline`'s was the most expensive of fifteen files, overstating that tool 2.6x. All three are now measured over baskets. That correction moved the aggregate by 0.1 points, because the per-tool errors were large and pointed opposite ways; the 9-point gain is a response-shaping fix to `search`.
...
- **{{ site.data.response_tokens.tools_costing_more }} of the {{ site.data.response_tokens.tools_with_baseline }} tools with a baseline return *more* tokens than it credits them** — `list_projects` at 3.9x is the worst, then `get_dead_code` and `get_complexity_report` at 2.3x, `check_claudemd_drift`, `search`, `get_feature_context`, `get_changed_symbols`, `find_usages`. The old counter booked a positive number for them anyway. The per-tool table is published in full: [tool response token cost](/perf/response-tokens/).
