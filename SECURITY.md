...
A third structural bucket is **`js/log-injection`** (`src/cli.ts`, `src/server/tool-filter.ts`, `src/cli/eval.ts`). The verdict is **accepted, dismissed as won't fix**, on two independent grounds. First, trust domain: trace-mcp is a local CLI/MCP server, and the "user-provided value" CodeQL tracks is the operator's own argument — a project root they typed, a preset name from their own config, a DB path derived from it. There is no privilege boundary between whoever supplies the value and whoever reads the log. Second, sink shape: the logger (`src/logger.ts`) is pino writing NDJSON to stderr with no pretty transport, so an interpolated value lands inside a JSON string and its newlines are escaped — a forged log record is not constructible through it, which is the actual harm this rule exists to prevent. The one non-pino site (`src/cli/eval.ts`, `console.error`) prints a `path.resolve()`d DB path to the operator's own terminal and is not machine-parsed. New `js/log-injection` alerts are checked against this shape and dismissed with the same justification. The exception is scoped to it: a value that reaches a log which something downstream *parses* — a shipped audit trail, an NDJSON stream consumed as a security record — is a real finding, and so is any log line assembled by string concatenation outside pino's structured fields.

The fourth is the pair **`js/prototype-polluting-assignment` / `js/remote-property-injection` in `src/registry.ts`** (`reg.projects[absRoot] = entry` and the in-place `existing.explicit = true` on the object that same key returns). Also **dismissed as won't fix**: `absRoot` is `path.resolve()` output, which is always an absolute path, so it can never be the string `__proto__`, `constructor`, or `prototype` — the flagged path is unreachable, not merely unlikely. `registry.json` is hand-editable, though, and the daemon holds the parsed map for its whole life, so `loadRegistry()` additionally deletes those three keys on every read (`DANGEROUS_KEYS`) rather than trusting the writer. Both the reachability argument and the scrub have to hold: an assignment into `reg.projects` under a key that is *not* `path.resolve()` output is a real finding, and removing the read-time scrub would make the file-tampering path live again.
