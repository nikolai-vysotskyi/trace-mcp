...
- **Anonymous** — a random install id (`~/.trace-mcp/telemetry-state.json`), the trace-mcp version, Node major version, OS platform, your machine's IANA timezone (e.g. `Europe/Berlin`), the name of the MCP client that connected (e.g. `claude-code`), and two aggregate counters since the previous ping: how many tool calls you made and the estimated tokens they saved (the same totals `trace-mcp savings` prints). No IP address is sent or overridden, and no project names, file paths, query content, per-tool or per-project breakdown, code, or anything else that could identify you or your codebase.
