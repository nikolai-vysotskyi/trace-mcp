...
|---|---|
| **Mixed real-world production** (measured tool responses vs. the file reads they replace) | **21% fewer tokens** |
...
**The 21% is the honest number to plan against, and it has gone down twice as we measured more of it.** We used to print "~40–50% on average". That figure descended from a counter that scored each call *before the tool ran* — `RAW_COST_ESTIMATES[tool] × 0.15`, a constant with no variance — which we found and fixed ourselves in [#915](https://github.com/nikolai-vysotskyi/trace-mcp/pull/915). The first honest replacement read 29.3% over twelve tools; measuring the remaining twelve took coverage from 88.4% to **97.2%** of recorded calls and took the figure down to 21%, because the tail held the worst per-call ratios in the product. It weights real `o200k_base` counts of real responses by 18,319 recorded calls from one machine ([per-tool table](https://trace-mcp.com/perf/response-tokens/), generated into `docs/_data/response_tokens.json`). Three caveats travel with it: the baseline half — what a `Read`/`Grep` would have cost instead — is **still a hand-written estimate**; **eight of the twenty-two tools measured return more tokens than they replace** — it was ten until the three worst were reshaped, and the old counter booked a saving for them anyway; and two more tools (`register_edit`, `reindex`) replace no file read at all, so they are now credited zero and counted as overhead — with them on the spend side the all-in figure is **19.4%**. The peaks below (up to 99% on individual structured calls) are a synthetic estimate, per-call, not per-session.

Measured at trace-mcp 3.18.0 (`94dbaf70`) on 5 September 2026. Its [preregistration](https://trace-mcp.com/perf/prereg-response-tokens/) publishes it as a **miss**, and this time on the headline half: the coverage bar is now cleared (97.2% against a declared 90%), but 21% does not clear the 25% we declared before measuring. We did not move the bar.
