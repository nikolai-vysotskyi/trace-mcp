...
|---|---|
| **Mixed real-world production** (measured tool responses vs. the file reads they replace) | **30.7% fewer tokens** |
...
**The 30.7% is the honest number to plan against, and it took three downward revisions and one measurement correction to get here.** We used to print "~40–50% on average". That figure descended from a counter that scored each call *before the tool ran* — `RAW_COST_ESTIMATES[tool] × 0.15`, a constant with no variance — which we found and fixed ourselves in [#915](https://github.com/nikolai-vysotskyi/trace-mcp/pull/915). The first honest replacement read 29.3% over twelve tools; measuring the remaining twelve took coverage from 88.4% to **97.2%** of recorded calls and took the figure down to 21%, because the tail held the worst per-call ratios in the product. Then the three busiest tools — 76% of all recorded calls — turned out to be published from **one sample each**: one query, one file, one query. `get_outline` was published from the most expensive of fifteen files, overstating it 2.6x. All three are now measured over fifteen-item baskets, and with that fixed the response-shaping work in [#1006](https://github.com/nikolai-vysotskyi/trace-mcp/pull/1006) takes the figure from 21.7% to **30.7%**. The sampling correction itself moved the aggregate by 0.1 points — the per-tool errors were large and pointed in opposite directions, so they cancelled. It weights real `o200k_base` counts of real responses by 18,319 recorded calls from one machine ([per-tool table](https://trace-mcp.com/perf/response-tokens/), generated into `docs/_data/response_tokens.json`). Three caveats travel with it, and clearing the bar retires none of them: the baseline half — what a `Read`/`Grep` would have cost instead — is **still a hand-written estimate**, so this is a measured numerator over an estimated denominator; **eight of the twenty-two tools measured return more tokens than they replace**, and the old counter booked a saving for them anyway; and two more tools (`register_edit`, `reindex`) replace no file read at all, so they are now credited zero and counted as overhead — with them on the spend side the all-in figure is **29%**. The peaks below (up to 99% on individual structured calls) are a synthetic estimate, per-call, not per-session.

Measured at trace-mcp 3.19.0 (`8ab72253`) on 6 September 2026. Its [preregistration](https://trace-mcp.com/perf/prereg-response-tokens/) publishes it as a **pass** for the first time after three misses — 30.7% against a declared 25%, over calls covering 97.2% against a declared 90%. The bar was declared unadjustable before any data and has not been moved. What cleared it is the response-shaping fix, not the sampling change: on the old sampling the same commit reads 30.8%.
