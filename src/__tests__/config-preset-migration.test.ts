/**
 * TRA-538: silent rewrite of the pre-TRA-402 `tools.preset: "full"` default.
 *
 * Configs written before the default moved still pin every session to the full
 * tool surface — the key is present, so the additive config migration never
 * touched it. The rewrite has to be silent, one-shot, and unable to undo a
 * deliberate `full`, which is what these tests pin down.
 */
...
describe('tools.preset default migration (TRA-538)', () => {
  ...
  const readPreset = () => {
    const parsed = parseJsonc(fs.readFileSync(configPath, 'utf8')) as {
      tools?: { preset?: string; preset_default_migrated?: boolean };
    };
    return parsed.tools ?? {};
  };
  ...
  it('rewrites our old default to the shipped one and marks the config', () => {
    ...
    expect(readPreset().preset_default_migrated).toBe(true);
