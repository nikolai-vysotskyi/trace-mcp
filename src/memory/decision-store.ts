...
export class DecisionStore {
  ...
  isSessionMined(sessionPath: string): boolean {
    return this.sessionOps.isSessionMined(sessionPath);
  }
  ...
  markSessionMined(sessionPath: string, decisionsFound: number): void {
    this.sessionOps.markSessionMined(sessionPath, decisionsFound);
  }
   ...
   *  - File shrank → `{ cursor: 0, reason: 'restart_shrunk' }`
  ...
  getSessionCursor(
    ...
    return this.sessionOps.getSessionCursor(sessionPath, currentSize, currentModifiedMs);
  }
  ...
  updateSessionCursor(opts: {
    ...
    this.sessionOps.updateSessionCursor(opts);
  }
  ...
  getProviderSessionCursor(
    ...
    return this.sessionOps.getProviderSessionCursor(
      sessionKey,
      currentSeenSize,
      currentSeenModifiedMs,
    );
  }
  ...
  updateProviderSessionCursor(opts: {
    ...
    this.sessionOps.updateProviderSessionCursor(opts);
  }

  // ── LLM EXTRACTION CACHE ──────────────────────────────────────────

  getCachedLlmExtraction(sessionId: string, contentSha: string, model: string): string | null {
    return this.sessionOps.getCachedLlmExtraction(sessionId, contentSha, model);
  }
  ...
  putCachedLlmExtraction(
    ...
    this.sessionOps.putCachedLlmExtraction(sessionId, contentSha, model, extractedJson);
  }
  ...
  addSessionChunks(chunks: SessionChunkInput[]): number {
    return this.sessionOps.addSessionChunks(chunks);
  }

  isSessionIndexed(sessionId: string): boolean {
    return this.sessionOps.isSessionIndexed(sessionId);
  }
  ...
  searchSessions(
    ...
    return this.sessionOps.searchSessions(query, opts);
  }

  getSessionChunkCount(projectRoot?: string): number {
    return this.sessionOps.getSessionChunkCount(projectRoot);
  }

  getIndexedSessionIds(projectRoot?: string): string[] {
    return this.sessionOps.getIndexedSessionIds(projectRoot);
  }
