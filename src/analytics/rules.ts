...
export interface OptimizationHit {
  rule: string;
...
const repeatedFileRead: Rule = {
  ...
  detect(calls) {
    ...
    for (const c of calls) {
      if (c.tool_name !== 'Read' && c.tool_name !== 'mcp__phpstorm__read_file' && c.tool_name !== 'mcp__phpstorm__get_file_text_by_path') continue;
    ...
    const repeated = [...fileCounts.entries()].filter(([, v]) => v.count >= 3).sort((a, b) => b[1].tokens - a[1].tokens);
    ...
    return {
      ...
      details: repeated.slice(0, 5).map(([k, v]) => `${k.split('::')[1]} (${v.count}x, ~${v.tokens} tokens)`),
      currentTokens,
      potentialTokens,
      recommendation: 'Use get_outline to understand file structure, then get_symbol for specific symbols instead of reading the full file repeatedly.',
    };
  },
};
...
const bashGrep: Rule = {
  ...
  detect(calls) {
    ...
    return {
      ...
      recommendation: 'Use trace-mcp `search` tool instead of Bash grep/rg. It returns structured, token-efficient results with symbol context.',
    };
  },
};
...
const bashCat: Rule = {
  ...
  detect(calls) {
    ...
    return {
      ...
      recommendation: 'Use get_symbol or Read tool instead of Bash cat/head/tail. These provide structured output and better token efficiency.',
    };
  },
};
...
const largeFileRead: Rule = {
  id: 'large-file-read',
  detect(calls) {
    const large = calls.filter(c =>
      (c.tool_name === 'Read' || c.tool_name === 'mcp__phpstorm__read_file' || c.tool_name === 'mcp__phpstorm__get_file_text_by_path')
    ...
    return {
      ...
      currentTokens,
      potentialTokens,
      recommendation: 'Use get_outline for file overview (signatures only), then get_symbol to read specific functions/classes.',
    };
  },
};
...
const phpstormReadIndexed: Rule = {
  ...
  detect(calls) {
    const phpstormReads = calls.filter(c =>
      (c.tool_name === 'mcp__phpstorm__read_file' || c.tool_name === 'mcp__phpstorm__get_file_text_by_path')
      && c.target_file,
    );
    ...
    return {
      ...
      currentTokens,
      potentialTokens,
      recommendation: 'Use trace-mcp get_symbol instead of PhpStorm file reading. It returns only the relevant symbol, saving tokens.',
    };
  },
};
...
const phpstormSearchIndexed: Rule = {
  ...
  detect(calls) {
    const searchTools = new Set(['mcp__phpstorm__search_in_files_by_text', 'mcp__phpstorm__search_in_files_by_regex', 'mcp__phpstorm__search_text', 'mcp__phpstorm__search_file']);
    ...
    return {
      ...
      currentTokens,
      potentialTokens,
      recommendation: 'Use trace-mcp `search` instead of PhpStorm text search. It understands symbols, FQNs, and language filters.',
    };
  },
};
...
const unusedTraceTools: Rule = {
  ...
  detect(calls) {
    ...
    return {
      ...
      recommendation: 'Enable trace-mcp tools for code navigation. Useful: search, get_outline, get_symbol, get_feature_context, find_usages.',
    };
  },
};
...
const agentForIndexed: Rule = {
  ...
  detect(calls) {
    const agentCalls = calls.filter(c =>
      c.tool_short_name === 'Agent' || c.tool_short_name === 'agent' ||
    ...
    return {
      ...
      potentialTokens,
      recommendation: 'Use get_feature_context or get_task_context instead of Agent subagents for code exploration. They return focused context within a token budget.',
    };
  },
};
...
export function analyzeOptimizations(toolCalls: ToolCallRow[], period: string): OptimizationReport {
  ...
  const savingsTokens = optimizations.reduce(
    (sum, o) => sum + (o.currentTokens - o.potentialTokens), 0,
  );
