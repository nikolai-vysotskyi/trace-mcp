...
export function registerRefactoringTools(server: McpServer, ctx: ServerContext): void {
  ...
  server.tool(
    ...
    async ({ file_path, start_line, end_line, function_name, dry_run }) => {
      ...
      const result = extractFunction(store, projectRoot, file_path, start_line, end_line, function_name, dry_run);
      if (!result.success) {
        return { content: [{ type: 'text', text: j(result) }], isError: true };
      }
      return { content: [{ type: 'text', text: j(result) }] };
    ...
    async ({ symbol_id, target_file, source_file, new_path, dry_run }) => {
      ...
      return {
        content: [{ type: 'text', text: j({
          success: false,
          error: 'Provide either (symbol_id + target_file) for symbol move, or (source_file + new_path) for file move',
        }) }],
        isError: true,
      };
    },
  ...
  server.tool(
    ...
    async ({ symbol_id, changes: rawChanges, dry_run }) => {
      const result = changeSignature(store, projectRoot, symbol_id, rawChanges as SignatureChange[], dry_run);
      if (!result.success) {
        return { content: [{ type: 'text', text: j(result) }], isError: true };
      }
      return { content: [{ type: 'text', text: j(result) }] };
  ...
  server.tool(
    ...
    'Preview any refactoring (rename, move, extract, signature) without applying. Returns all edits as {old_text, new_text} pairs. Use to review changes before applying the real tool.',
    {
      ...
      changes: z.array(planSignatureChangeSchema).optional().describe('Signature changes (for signature)'),
    },
    async (params) => {
