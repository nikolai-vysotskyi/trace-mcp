...
export function registerRefactoringTools(server: McpServer, ctx: ServerContext): void {
  ...
  server.tool(
    'extract_function',
    'Extract a line range out of an enclosing function into a new named helper (AST-aware, TypeScript/JavaScript). Computes the parameter list via free-variable analysis and a return value from a binding used after the slice; the slice is replaced by a call to the new helper. Dry-run by default. Returns { success, edits, extracted_params, return_value, confidence, files_modified }.',
    {
  ...
  server.tool(
    'apply_codemod',
    'Structural (AST-aware) or regex find-and-replace across files. Engine "auto" (default) uses ast-grep syntax matching for supported code files when the pattern looks like an ast-grep pattern (skips strings/comments), else falls back to text regex. Dry-run by default; potentially destructive once dry_run=false. Always preview first. Returns { success, engine_used, dry_run, matches, files_modified, total_replacements, total_files }.',
    {
      pattern: z
        .string()
        .min(1)
