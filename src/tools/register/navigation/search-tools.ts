...
export function registerSearchTools(server: McpServer, ctx: ServerContext): void {
  ...
  server.tool(
    'search',
    'Search symbols by name, kind, or text — the primary code-lookup tool. Use search_text for raw text/comments, find_usages for references to a known symbol. Supports fuzzy, semantic, and fusion ranking, plus kind/language/path/implements/extends/decorator filters (see param docs). Read-only. Returns { items: [{ symbol_id, name, kind, fqn, signature, file, line, score }], total, search_mode }. Set output_format: "toon" for lossless token savings.',
    {
