...
export function registerContextTools(server: McpServer, ctx: ServerContext): void {
  ...
  server.tool(
    'get_feature_context',
    'Search code by keyword/topic → returns ranked source code snippets within a token budget. Use when you need to READ actual code for a concept or feature. For structured task context with tests and entry points use get_task_context instead; for symbol metadata without source use search. Read-only, capped by `memory.recall.timeoutMs` (degrades gracefully on timeout). Returns { items: [{ symbol_id, name, file, source, score }], token_usage }. Set output_format: "toon" for lossless token savings.',
    {
      description: z
        .string()
        .min(1)
