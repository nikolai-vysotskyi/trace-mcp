...
export function registerQualityTools(server: McpServer, ctx: ServerContext): void {
  ...
  // --- Co-Change Analysis ---
  server.tool('get_co_changes', 'Find files that frequently change together in git history (temporal coupling). Requires git. Use to discover hidden dependencies between files. For cross-module co-change anomalies use detect_drift instead. Read-only. Returns JSON: { file, coChanges: [{ file, confidence, count }] }.', { file: z.string().min(1).max(512).describe('File path to analyze'), min_confidence: z.number().min(0).max(1).optional().describe('Minimum confidence threshold (default 0.3)'), min_count: z.number().int().min(1).optional().describe('Minimum co-change count (default 3)'), window_days: z.number().int().min(1).max(730).optional().describe('Git history window in days (default 180)'), limit: z.number().int().min(1).max(100).optional().describe('Max results (default 20)') }, async ({ file, min_confidence, min_count, window_days, limit: lim }) => { const result = getCoChanges(store, { file, minConfidence: min_confidence, minCount: min_count, windowDays: window_days, limit: lim }); if (result.isErr()) return { content: [{ type: 'text', text: j(formatToolError(result.error)) }], isError: true }; return { content: [{ type: 'text', text: j(result.value) }] }; });
  server.tool('refresh_co_changes', 'Rebuild co-change index from git history. Mutates the co-change index; idempotent. Use after significant git history changes. Returns JSON: { status, pairs_stored, window_days }.', { window_days: z.number().int().min(1).max(730).optional().describe('Git history window in days (default 180)') }, async ({ window_days }) => { const days = window_days ?? 180; const pairs = collectCoChanges(projectRoot, days); const count = persistCoChanges(store, pairs, projectRoot, days); return { content: [{ type: 'text', text: j({ status: 'completed', pairs_stored: count, window_days: days }) }] }; });
  // --- Changed Symbols ---
  server.tool('get_changed_symbols', 'Map a git diff to affected symbols (functions, classes, methods). For PR review. If "since" is omitted, auto-detects main/master as the base. Requires git. Use for PR review to see which symbols changed. For full branch comparison with risk assessment use compare_branches instead. Read-only. Returns JSON: { changes: [{ symbol_id, name, kind, file, changeType }], total }.', { since: z.string().min(1).max(256).optional().describe('Git ref to compare from (SHA, branch, tag). If omitted, auto-detects main/master merge-base'), until: z.string().max(256).optional().describe('Git ref to compare to (default: HEAD)'), include_blast_radius: z.boolean().optional().describe('Include blast radius for each changed symbol (default false)'), max_blast_depth: z.number().int().min(1).max(10).optional().describe('Max blast radius traversal depth (default 3)') }, async ({ since, until, include_blast_radius, max_blast_depth }) => { const result = getChangedSymbols(store, projectRoot, { since, until, includeBlastRadius: include_blast_radius, maxBlastDepth: max_blast_depth, defaultBaseBranch: config.git?.defaultBaseBranch }); if (result.isErr()) return { content: [{ type: 'text', text: j(formatToolError(result.error)) }], isError: true }; return { content: [{ type: 'text', text: j(result.value) }] }; });
  ...
  // --- Community Detection ---
  server.tool('detect_communities', 'Run Leiden community detection on the file dependency graph. Identifies tightly-coupled file clusters (modules). Mutates the community index (stores results); idempotent. Use before get_communities or get_community. Returns JSON: { communities: [{ id, files, size }], modularity }.', { resolution: z.number().min(0.1).max(5).optional().describe('Resolution parameter — higher values produce more communities (default 1.0)') }, async ({ resolution }) => { const result = detectCommunities(store, resolution ?? 1.0); if (result.isErr()) return { content: [{ type: 'text', text: j(formatToolError(result.error)) }], isError: true }; return { content: [{ type: 'text', text: j(result.value) }] }; });
  server.tool('get_communities', 'Get previously detected communities (file clusters). Run detect_communities first. Read-only. Returns JSON: { communities: [{ id, files, size }], total }.', {}, async () => { const result = getCommunities(store); if (result.isErr()) return { content: [{ type: 'text', text: j(formatToolError(result.error)) }], isError: true }; return { content: [{ type: 'text', text: j(result.value) }] }; });
  server.tool('get_community', 'Get details for a specific community: files, inter-community dependencies. Read-only. Use after detect_communities to drill into a specific cluster. Returns JSON: { id, files, interCommunityDeps }.', { id: z.number().int().min(0).describe('Community ID') }, async ({ id }) => { const result = getCommunityDetail(store, id); if (result.isErr()) return { content: [{ type: 'text', text: j(formatToolError(result.error)) }], isError: true }; return { content: [{ type: 'text', text: j(result.value) }] }; });

  // --- Audit Config ---
  server.tool('audit_config', 'Scan AI agent config files for stale references, dead paths, and token bloat. Read-only. Use periodically to clean up CLAUDE.md and settings. Returns JSON: { issues: [{ file, type, message, suggestion }], total }.', { config_files: z.array(z.string().max(512)).optional().describe('Specific config files to audit (default: auto-detect)'), fix_suggestions: z.boolean().optional().describe('Include fix suggestions (default true)') }, async ({ config_files, fix_suggestions }) => { const result = auditConfig(store, projectRoot, { configFiles: config_files, fixSuggestions: fix_suggestions ?? true }); return { content: [{ type: 'text', text: j(result) }] }; });
