...
export function findDuplicateSymbols(
  ...
  const sourceIsTest = TEST_PATH_RE.test(sourceFilePath);
  ...
  const checkable = sourceSymbols
    .filter((s) =>
      CHECKABLE_KINDS.has(s.kind)
      && s.name.length >= MIN_NAME_LENGTH
      && !TRIVIAL_NAMES.has(s.name),
    )
  ...
  for (const src of checkable) {
    ...
    for (const cand of candidates) {
      ...
      const candIsTest = TEST_PATH_RE.test(candFile.path);
      ...
      const candParamCount = candSymbol?.param_count
        ?? paramCountStmt.get(cand.symbolId)?.param_count ?? null;
