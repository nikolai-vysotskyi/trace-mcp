...
export function buildProjectContext(rootPath: string): ProjectContext {
  ...
  const readFile = (rel: string): string | undefined => {
    try { return fs.readFileSync(path.resolve(rootPath, rel), 'utf-8'); } catch { return undefined; }
  };
  ...
  if (pkgRaw) {
    try {
      ...
      if (engines) {
        if (engines.node) detectedVersions.push({ runtime: 'node', version: engines.node, source: 'package.json#engines.node' });
  ...
  if (tomlRaw) {
    ...
    if (pyReq) detectedVersions.push({ runtime: 'python', version: pyReq[1], source: 'pyproject.toml#requires-python' });
  ...
  if (pyVer) detectedVersions.push({ runtime: 'python', version: pyVer, source: '.python-version' });
  ...
  if (reqRaw) {
    ...
    requirementsTxt = lines.map((l) => l.split(/[>=<!\[;]/)[0].trim().toLowerCase());
    for (const l of lines) {
      const parts = l.match(/^([a-zA-Z0-9_.-]+)\s*(.*)/);
      if (parts) allDependencies.push({ name: parts[1].toLowerCase(), version: parts[2] || undefined });
    }
  }
  ...
  if (goModRaw) {
    ...
    if (goVerMatch) detectedVersions.push({ runtime: 'go', version: goVerMatch[1], source: 'go.mod' });
  ...
  if (cargoRaw) {
    ...
    for (const [sectionName, dev] of [['dependencies', false], ['dev-dependencies', true]] as const) {
      const sectionRe = new RegExp(`\\[${sectionName}\\]([^\\[]*)`, 'g');
      let sm: RegExpExecArray | null;
      while ((sm = sectionRe.exec(cargoRaw)) !== null) {
        for (const line of sm[1].split('\n')) {
          const dep = line.match(/^\s*([a-zA-Z0-9_-]+)\s*=\s*(?:"([^"]+)"|.*version\s*=\s*"([^"]+)")/);
          if (dep) cargoDeps.push({ name: dep[1], version: dep[2] ?? dep[3], dev: dev || undefined });
        }
      }
    }
    const editionMatch = cargoRaw.match(/edition\s*=\s*"(\d{4})"/);
    if (editionMatch) detectedVersions.push({ runtime: 'rust', version: `edition-${editionMatch[1]}`, source: 'Cargo.toml#edition' });
    const rustVersionMatch = cargoRaw.match(/rust-version\s*=\s*"([^"]+)"/);
    if (rustVersionMatch) detectedVersions.push({ runtime: 'rust', version: rustVersionMatch[1], source: 'Cargo.toml#rust-version' });
  ...
  if (rubyVer) detectedVersions.push({ runtime: 'ruby', version: rubyVer.replace(/^ruby-/, ''), source: '.ruby-version' });
  ...
  if (pomRaw) {
    ...
    const depMatches = pomRaw.matchAll(/<dependency>\s*<groupId>([^<]+)<\/groupId>\s*<artifactId>([^<]+)<\/artifactId>(?:\s*<version>([^<]+)<\/version>)?/g);
    ...
    const javaSource = pomRaw.match(/<maven\.compiler\.source>([^<]+)<\/maven\.compiler\.source>/)?.[1]
      ?? pomRaw.match(/<java\.version>([^<]+)<\/java\.version>/)?.[1]
      ?? pomRaw.match(/<release>([^<]+)<\/release>/)?.[1];
    if (javaSource) detectedVersions.push({ runtime: 'java', version: javaSource, source: 'pom.xml' });
  ...
  if (gradleRaw) {
    ...
    const depLines = gradleRaw.matchAll(/(?:implementation|api|compileOnly|runtimeOnly|testImplementation)\s*[("']([^)'"]+)[)'"]/g);
    ...
    const javaSrcCompat = gradleRaw.match(/sourceCompatibility\s*=\s*['"]?([^'"\s\n]+)/)?.[1]
      ?? gradleRaw.match(/JavaVersion\.VERSION_(\d+)/)?.[1];
    if (javaSrcCompat) detectedVersions.push({ runtime: 'java', version: javaSrcCompat, source: 'build.gradle' });
  ...
  if (toolVersions) {
    for (const line of toolVersions.split('\n')) {
      ...
      if (parts) {
        const runtimeMap: Record<string, string> = { nodejs: 'node', python: 'python', ruby: 'ruby', golang: 'go', java: 'java', rust: 'rust' };
  ...
  const CONFIG_FILE_NAMES = [
    ...
    'app.config.ts', 'app.config.js',       // Nuxt UI theme
