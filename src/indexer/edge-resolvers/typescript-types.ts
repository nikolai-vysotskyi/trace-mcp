...
export function resolveTypeScriptTypeEdges(state: PipelineState): void {
  ...
  const refEdgeType = store.db.prepare(
  ...
  const targetSyms = store.db.prepare(`
    ...
    for (const src of sources) {
      ...
      for (const refName of typeRefs) {
        const target = resolveTypeRef(
          refName, src.workspace, src.file_id, fileImports, byName, targetSyms,
        );
        if (!target) continue;
        ...
        insertStmt.run(
          sourceNodeId, targetNodeId, refEdgeType.id,
