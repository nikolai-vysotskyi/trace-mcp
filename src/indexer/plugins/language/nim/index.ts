...
const _plugin = createRegexLanguagePlugin({
  ...
  symbolPatterns: [
    ...
    {
      kind: 'class',
      pattern: /^\s{2,}(\w+)\*?\s*(?:\[.*?\])?\s*=\s*(?:ref\s+)?object\b/gm,
      isScope: true,
    },
    // Enum types: Name* = enum
    {
      kind: 'type',
      pattern: /^\s{2,}(\w+)\*?\s*=\s*enum\b/gm,
      meta: { enum: true },
      isScope: true,
    },
    ...
    {
      kind: 'interface',
      pattern: /^\s{2,}(\w+)\*?\s*=\s*concept\b/gm,
      meta: { concept: true },
      isScope: true,
    },
    ...
    {
      kind: 'type',
      pattern: /^\s{2,}(\w+)\*?\s*=\s*(?!object|enum|concept|distinct|tuple|ref\s)\w/gm,
    },
