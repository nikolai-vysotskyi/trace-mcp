  ...
  extractNodes(
    ...
    if (mainHandles.length + mainHandleOnces.length + mainOns.length + mainOnces.length > 0) {
      ...
      for (const channel of mainHandles) {
        result.edges!.push({
          source: filePath,
          target: channel,
          edgeType: 'electron_ipc_handle',
          metadata: { channel, variant: 'handle' },
        });
      }
      for (const channel of mainHandleOnces) {
        result.edges!.push({
          ...
          target: channel,
          edgeType: 'electron_ipc_handle',
          metadata: { channel, variant: 'handleOnce' },
        });
      }
      for (const channel of mainOns) {
        result.edges!.push({
          source: filePath,
          target: channel,
          edgeType: 'electron_ipc_main_on',
          metadata: { channel, variant: 'on' },
        });
      }
    ...
    for (const channel of frameOns) {
      result.edges!.push({
        ...
        target: channel,
        edgeType: 'electron_ipc_main_on',
        metadata: { channel, variant: 'frame_scoped' },
      });
    }
    ...
    const hasRendererIpc =
      rendererInvokes.length +
      rendererSends.length +
      rendererSendSyncs.length +
      rendererSendToHosts.length +
      rendererPostMsgs.length +
      rendererOns.length +
      rendererOnces.length;
    ...
    if (hasRendererIpc > 0) {
      ...
      for (const channel of rendererSendSyncs) {
        result.edges!.push({
          source: filePath,
          target: channel,
          edgeType: 'electron_ipc_send_sync',
          metadata: { channel },
        });
        warnings.push(
      ...
      for (const channel of rendererSendToHosts) {
        result.edges!.push({
          ...
          target: channel,
          edgeType: 'electron_ipc_send',
          metadata: { channel, variant: 'sendToHost' },
        });
      }
      for (const channel of rendererPostMsgs) {
        result.edges!.push({
          ...
          target: channel,
          edgeType: 'electron_ipc_send',
          metadata: { channel, variant: 'postMessage' },
        });
      }
    ...
    for (const m of source.matchAll(CONTEXT_BRIDGE_RE)) {
      ...
      result.edges!.push({
        source: filePath,
        target: m[1],
        edgeType: 'electron_preload_api',
        metadata: { apiName: m[1] },
      });
    }
    if (CONTEXT_BRIDGE_ISOLATED_RE.test(source)) {
      ...
      result.edges!.push({
        source: filePath,
        target: 'isolatedWorld',
        edgeType: 'electron_preload_api',
        metadata: { variant: 'isolatedWorld' },
      });
    }
    ...
    if (BROWSER_WINDOW_RE.test(source)) {
      ...
      result.edges!.push({
        ...
        edgeType: 'electron_browser_window',
        metadata: { type: 'BrowserWindow' },
      });
    }
    ...
    for (const m of source.matchAll(UTILITY_FORK_RE)) {
      ...
      result.edges!.push({
        source: filePath,
        target: modulePath,
        edgeType: 'electron_utility_fork',
        metadata: { modulePath },
      });
    }
    ...
    for (const m of source.matchAll(PROTOCOL_HANDLE_RE)) {
      result.edges!.push({
        source: filePath,
        target: m[1],
        edgeType: 'electron_protocol_handle',
        metadata: { scheme: m[1] },
      });
    }
