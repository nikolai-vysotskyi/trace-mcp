...
// import patterns
const IMPORT_RE = /^\s*(?:from\s+(requests|httpx|aiohttp)(?:\.\w+)?\s+import|import\s+(requests|httpx|aiohttp))\b/m;
...
export class PythonHttpClientsPlugin implements FrameworkPlugin {
  ...
  registerSchema() {
    return {
      edgeTypes: [
        { name: 'http_outbound', category: 'http', description: 'Outgoing HTTP call (requests/httpx/aiohttp)' },
      ],
    };
  }
  ...
  extractNodes(
    ...
    if (!IMPORT_RE.test(source) && !source.includes('requests.') && !source.includes('httpx.') && !source.includes('aiohttp.')) {
      return ok({ status: 'ok', symbols: [] });
    }
    ...
    if (/\baiohttp\b/.test(source)) {
      ...
      const hasSession = AIOHTTP_SESSION_RE.test(source) || /\baiohttp\s*\.\s*request\s*\(/.test(source);
      if (hasSession) {
      ...
      for (const m of source.matchAll(/\baiohttp\s*\.\s*request\s*\(\s*["']([A-Z]+)["']\s*,\s*(?:(?:f|r|b)?["']([^"']+)["'])?/g)) {
        emit('aiohttp', m[1], m[2], findLine(m.index ?? 0));
      }
    }
