...
| A baseline is written only from a reading the app actually has | The KPI snapshot was guarded on `metricsLoading` alone, which goes **false the moment the request fails** — so a launch against a daemon that never answered stored an all-zero baseline and the dashboard reported the whole workspace as this afternoon's growth: "656.2k symbols, ↑ +656.2k". A delta that equals its own value is a bare number wearing an arrow, so a zero-project snapshot is neither written nor compared against. |
