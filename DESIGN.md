...
| A sentence borrowed for its **tail** brings its head along, and the head is a claim | Project Overview's Index card printed the Workspace's `busyStale` — "The daemon is busy. These are the last indexed numbers." — whenever its numbers came from the snapshot rather than a fresh answer. Only the second half is what that flag knows. The first half is a verdict, chosen on the Workspace by `busyMessage` from a real busy reading and asserted here for every stale one, directly above the Status row that names the actual condition: on the running app it read "The daemon is busy." over "Daemon unreachable", then over "Not tracked" a poll later. Reusing a string to avoid two wordings for one condition is right; reusing it across two *different* conditions is how a card ends up with two verdicts. A shared string has to be true of every predicate that renders it — the cause belongs to whichever element owns status, and the borrowed line describes only its own subject. |
...
| A baseline needs an **age**, not just a value — a comparison against a minute ago is the number repeated | The daily baseline was shown the moment it was stored, so a first launch read "No change vs 34 seconds ago" on two tiles and "↑ +3 vs 34 seconds ago" on the third — the three projects that finished loading after the snapshot, reported as workspace growth. Both are the §7 failure the comparison line exists to prevent, in the same shape as the empty-snapshot one above: the value restated with a caption on it. A snapshot younger than an hour is kept but not compared against, and the tile falls back to the no-baseline state it already has. The caption is computed at render, never memoized on the baseline: the baseline rolls once per mount, so a memoized age froze at mount time while the delta number kept tracking the 2s poll — a window left open all day still said "vs 34 seconds ago" over a whole day of growth. |
