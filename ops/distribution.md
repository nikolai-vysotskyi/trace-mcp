...
| [natsukium/mcp-servers-nix](https://github.com/natsukium/mcp-servers-nix) | **No — asked 2026-09-06** | — | An **install path, not a listing**: a package there is `nix run` / `nix profile` plus a module that writes the server into an agent's config, so the arrivals objection to directory rows does not apply. `serena` is already packaged and the maintainer's bot bumps it almost daily (PRs #595–#605 in two weeks), so one merged package is a maintained channel rather than a one-off row. 297★, pushed daily, `CONTRIBUTING.md` has an "Adding a New Package" section with three categories (`official` = upstream authors, i.e. ours). Package requests by issue are an accepted form there (#128, #575). **We cannot open the PR unaided:** `fetchFromGitHub` and `npmDepsHash` are NAR hashes and there is no Nix on the machine — installing Nix on Nikolai's machine for this is a machine-level change we do not make. Four real packaging costs, all disclosed in the request: four prebuilt-binary native deps (`better-sqlite3`, `@ast-grep/napi`, `@parcel/watcher`, `oxc-resolver`); `postinstall-control-plane.mjs` writing `~/.trace/` and a macOS LaunchAgent (opt-outs `TRACE_MCP_NO_POSTINSTALL=1` / `TRACE_MCP_NO_AUTO_UPDATE=1` exist and the server runs without all three scripts); pnpm rather than npm; two genuinely optional deps. Tracked in TRA-1012 | 2026-09-06 |
