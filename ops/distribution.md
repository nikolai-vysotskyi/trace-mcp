...
**The `trace` rename does not touch a single listing (TRA-610 / TRA-615,
2026-09-01).** What is shortening is the CLI command, the MCP server key clients
write into their configs, and `~/.trace-mcp/` → `~/.trace/`. Three identifiers
every surface in the table above keys off are deliberately **not** changing:

- **The npm package stays `trace-mcp`.** `trace` on npm is taken — `andreasmadsen`'s
  "Creates super long stack traces", 3.2.0, unrelated to us (checked
  `npm view trace` on 2026-09-01). Every install line we publish anywhere stays
  `npm install -g trace-mcp` / `npx -y trace-mcp@latest`. A listing that says
  `npm install -g trace` sends our users to someone else's package; treat that as
  a P0 correction wherever it appears, including in our own docs.
- **`server.json` stays `io.github.nikolai-vysotskyi/trace-mcp`**, so the official
  registry row and everything downstream of it (mcp.so, PulseMCP) is unaffected.
- **Plugin ids stay `trace-mcp`** — `@nikolai-vysotskyi/trace-mcp` for Claude Code,
  `nikolai-vysotskyi/trace-mcp` for the Codex marketplace.

So: no re-submission anywhere, and no row above needs re-verifying because of the
rename. The one thing that does propagate is README prose, which glama.ai scrapes
live — and that follows on its own once the release is out, per the "listing fix
ships with the next release" note above.

There is also no such thing as a server that answers to both names. The server
registers one name; what is dual is the *client-side* config key, because an entry
a user leaves spelled `trace-mcp` keeps pointing at the same binary. Do not write
"supports both server names" into a listing — it is not a feature we have.
