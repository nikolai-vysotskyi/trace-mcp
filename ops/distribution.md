...
|---|---|---|---|---|
| [registry.modelcontextprotocol.io](https://registry.modelcontextprotocol.io) | Yes — `io.github.nikolai-vysotskyi/trace-mcp` | Current: 3.1.1, correct counts. Supports both `trace` and `trace-mcp` server names | Automatic: `.github/workflows/publish-mcp-registry.yml` republishes `server.json` on every release (GitHub OIDC, no secret) | 2026-09-01 |
| [glama.ai](https://glama.ai/mcp/servers/nikolai-vysotskyi/trace-mcp) | Yes | Correct — scrapes README/npm live (`trace` and `trace-mcp` aliases) | Nothing to do; fix the README and it follows | 2026-09-01 |
| [pulsemcp.com](https://www.pulsemcp.com/servers/nikolai-vysotskyi-trace) | Yes | **Stale: "44+ tools"** — their hand-written `server.json`, kept "until the maintainer publishes to the official registry" | Their submissions are **paused**; their own submit page says publishing to the official registry is the fix. Done 2026-08-29 — waiting on their next sync | 2026-09-01 |
...
**`trace` binary alias and token reduction (TRA-610 / TRA-615, 2026-09-01).**
The package exposes dual CLI binaries (`trace` and `trace-mcp`) and supports dual
MCP server registration names (`trace` and `trace-mcp`). The motivation is token
efficiency: `trace` is a single BPE token across cl100k, o200k, Claude, and Gemini
tokenizers (vs 3–4 tokens for `trace-mcp`), cutting mention overhead by 60–75%.
In MCP client schemas, `mcp__trace__<tool>` saves prefix tokens on every advertised
tool. All existing configs (`.trace.json` / `.trace-mcp.json`), data directories
(`~/.trace/` / `~/.trace-mcp/`), and MCP client setups are 100% backwards-compatible
