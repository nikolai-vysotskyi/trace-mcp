...
| [crazy54/awesome-agentic-atlas](https://github.com/crazy54/awesome-agentic-atlas) | **No — rejected 2026-09-16, link-farm shape** | Created 2026-09-03, 0 stars / 0 forks, one contributor, recent commits are CNAME edits and self-merges. An aggregator of aggregators ("guide to all the top Awesome repos") with no audience and no track record — the dofollow-link shape the rules at the top of this file warn about. Auto-listed us 2026-09-21 (found by the sweep, not submitted); still 0★ / 0 forks — rejection stands. Do not re-evaluate before it has independent contributors or a measurable audience | 2026-09-22 |
| [mrchatam/Trace](https://github.com/mrchatam/Trace) | **No — direct competitor, no outreach** | Go knowledge-graph MCP for coding agents (CLI + MCP + optional GUI) shipping binaries literally named `trace` and `trace-mcp` — 12★, created 2026-08-15, pushed 2026-09-20, no reference to us anywhere (name collision, not a mention). Per the competitor rule: no contact, recorded here so the next pass does not re-read it. Flagged to Competitor Intelligence 2026-09-21. Independently useful as evidence: a same-named binary on the ecosystem floor is exactly the PATH-confusion case the MiniMax reviewer raised (row below) | 2026-09-22 |
| [MiniMax-AI/MiniMax-Code-Plugins](https://github.com/MiniMax-AI/MiniMax-Code-Plugins) | **Submitted 2026-09-19, in review** — [PR #54](https://github.com/MiniMax-AI/MiniMax-Code-Plugins/pull/54) (`nikolai-vysotskyi:add-trace-mcp-plugin`) | Plugin dir `plugins/nikolai-vysotskyi/trace-mcp`: README, `mcp.json`, `plugin.json` (3.28.0), 4 skills. Reviewer `hetaoBackend` (a plugin author in the same repo) gave two `CHANGES_REQUESTED` rounds (2026-09-21, 2026-09-22); every point fact-checked true against the pinned tree and answered in-thread with pushed fixes — round 1: version pin, GA4/telemetry disclosure, Writes section; round 2 (commit `810585b`): `mcp.json` runs `npx -y trace-mcp@3.28.0` so the registry-resolved pinned version answers every launch (verified against a fake same-named `PATH` binary), README lists all six mutating tools with exact gates plus the two stated limits (no automatic rollback, project-root confinement without symlink claims), isolated MCP smoke evidence posted (initialize 3.28.0, 49-edit dry-run preview byte-identical, >20-file apply fails closed, extract applies, telemetry off leaves no state). Do not ping — the reviewer is actively engaged (two rounds in 36h). Tracked in TRA-1785 / TRA-1814 | 2026-09-22 |
...
### Pass 2026-09-22 (TRA-1814): MiniMax round 2 answered with smoke evidence, sweep classified, no new submissions

**Open threads (all via API). One needed action, rest silent.**
- `MiniMax-AI/MiniMax-Code-Plugins#54`: second `CHANGES_REQUESTED` from `hetaoBackend` arrived 2026-09-22T01:38Z (four points). Answered same run: pushed `810585b` to our fork branch (npx-pinned `mcp.json`, full six-tool Writes section with stated limits, skill param fix) and replied in-thread with isolated smoke transcript against pinned 3.28.0. Ledger table row added above (was missing — the submission existed since 2026-09-19 with no row).
- `eltociear#119`, `GetBindu#195` (open, clean), `tolkonepiu#384` (open, 0 comments, no ping before 26.09), `ai-boost#240` (open, dirty again — left per standing rule), `hermes#395` (open; triage batch closed #421 on 09-17, not ours), `nix#606` (open; our 09-20 comment stands as the one reminder, no second), `awesome-claude-code#2871` (open, bot only, day 4 — no ping), `Dokploy/mcp#81` (open, newest still the 09-05 brush-off — nothing owed), `mattbutlerengineering/ai-tooling#585`, `pi-extensions#1204`: unchanged.
- trace-mcp repo: 178★ / 21 forks (+2★ since 09-19).

**Sweep:** 3 hits, all classified, all in `mentions-seen.txt` now — `mrchatam/Trace` (competitor, ledger row above), `crazy54` (auto-listed us 09-21, rejection stands), `yzfly` (own merged entry). Doc bug found while answering the review: upstream `skills/trace-mcp-refactoring/SKILL.md` extract example uses `new_name`, schema takes `function_name` — fixed in the MiniMax PR only, upstream fix left for Docs/Implementation.

**Catalogs:** moratorium holds, 0 new submissions. Queue unchanged (aloth, TensorBlock).

**Touch budget: 0 of 5 new used.** The MiniMax reply and push continue our own open thread (not a new contact).
