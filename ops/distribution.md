...
| [0xNyk/awesome-hermes-agent](https://github.com/0xNyk/awesome-hermes-agent) | **Submitted 2026-09-06, awaiting triage** | — | [Issue #395](https://github.com/0xNyk/awesome-hermes-agent/issues/395), category `Tools & Utilities`, next to `llmtrim` and `agentburn`. 5,574★, pushed 2026-09-02, CC BY 4.0-ish (`NOASSERTION`), no payment and no ownership badge. **PRs are forbidden in writing** — `CONTRIBUTING.md`: "Please do not open a PR directly to add a resource"; the route is the `resource-submission.yml` issue form, which we reproduced field by field (`gh` cannot render the form, so the body mirrors its headings). Closing an issue there means *listing*, not refusal — `ai-usage-monitor` (#362, closed 2026-08-28) is in the README today. Triage is batched: four closed 2026-09-02, five 2026-08-28, and nine submissions are open ahead of us. The submission discloses three things on purpose, because the list has a trust-boundary section: the `shell-hooks-allowlist.json` write on standard/max installs, the missing uninstall path for the `pre_tool_call` hook, and that the Hermes surfaces are tested against the documented schema but never against a live install (`agentburn` disclosed the same and was accepted). **Do not re-submit and do not ping before 2026-09-20** | 2026-09-06 |
...
| [0xNyk/awesome-hermes-agent](https://github.com/0xNyk/awesome-hermes-agent) | **Submitted 2026-09-06** — [issue #394](https://github.com/0xNyk/awesome-hermes-agent/issues/394) | Requested section `Tools & Utilities`, beside `llmtrim` and `rtk-hermes` | **Issue, never a PR** — `CONTRIBUTING.md` says so outright ("Please do not open a PR directly to add a resource"); approved entries land in batch triage commits by the maintainer. Form is `resource-submission.yml` (name / URL / author / category / description / why awesome / why now / license / maturity / disclosures). Door verified by outcome, not by policy text: issue #362 (`ai-usage-monitor`) was closed 2026-08-28 and the tool is in the README today, so a closed submission means listed. Batches on 2026-08-28 and 2026-09-02. 5,574★, CC BY 4.0, no payment, no ownership claim, no badge asked of us. **Why this one is not a generic MCP listing:** Hermes is a supported client in our own code — `init` writes `mcp_servers` into `$HERMES_HOME/config.yaml` and the `AGENTS.md` block, installs one `pre_tool_call` hook with matcher `terminal`, and reads `state.db` plus the per-profile DBs read-only for `discover_hermes_sessions` / `mine_sessions`. Tracked in TRA-1010 | 2026-09-06 |
