...
| [cursor.directory](https://cursor.directory) / Agent Plugins (`agent-plugins.org`) | **Ready for submission** | None | Auto-detects components via Agent Plugins v1.0.0 (`plugin.json`, `mcp.json`, `skills/*`) | Repo root conforms to Agent Plugins spec (TRA-634). Submission via web form at `cursor.directory/plugins/new` requires human GitHub OAuth login (Nikolai). Once submitted, auto-detection ingests the MCP server and skills automatically | 2026-09-02 |
...
- **cursor.directory / Agent Plugins** — the submission form at `cursor.directory/plugins/new`
  requires GitHub or Google OAuth login. Once submitted, the crawler auto-detects
  our root `plugin.json`, `mcp.json`, and `skills/*` without manual entry.
