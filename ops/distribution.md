...
| [mattbutlerengineering/ai-tooling](https://github.com/mattbutlerengineering/ai-tooling/blob/main/evaluations/trace-mcp.md) | **No — account gone 2026-09-22** | Was a 2026-06-22 hands-off source review of v1.43.1 (verdict `discovery-log — tentative read`), the only third-party evaluation of trace-mcp anyone had written. On 2026-09-22 the whole account 404s: `gh api users/mattbutlerengineering`, the repo, and issue #585 all return Not Found (API verified working against other users the same minute; web fetch of the profile and repo also 404). Web search still carries stale index entries — do not trust them. Our correction [#585](https://github.com/mattbutlerengineering/ai-tooling/issues/585) (telemetry claim, filed 2026-09-04) is moot with it; TRA-857's no-follow-up rule is now moot too — there is nobody to write to. If the account reappears under a new name, re-verify before restoring this row | 2026-09-22 |
...
### Delta recheck, 2026-09-22 evening (TRA-1824): 32 threads rechecked, one surface died, one loop closed

Four hours after the day pass (TRA-1819), all via GitHub API. `updated_at` on every thread predates the day pass except the two findings below.

**Finding 1 — a surface died: `mattbutlerengineering/ai-tooling` is gone.** Account, repo and our correction issue #585 all 404 (API + web fetch, same minute; API verified working against other users). Table row above updated to dead with the verification. Nothing to submit, nobody to write to, no follow-up possible — TRA-857's rule is moot. Note for the next run: web search still returns stale index entries for the account; do not trust them, and if the account reappears under a new name re-verify before restoring the row.

**Finding 2 — a loop closed itself: `schalermthai/redproof#50` maintainer replied 2026-09-21T01:28Z.** Thanks for the `@ast-grep/napi` notes, busy with the day job, explicitly letting it sit ("If I hit a Gate that text tools really can't prove, I'll pick it back up, and your notes will save me a lot of pain"). No question asked, no reply owed — a thank-you would only cost a notification. Recorded here so the next run does not re-read it as news.

**Everything else silent, nothing owed, no pings due.** MiniMax#54: our 04:12Z answer and `810585b` push still last in thread, no reviewer reaction — wait. `awesome-claude-code#2871`: bot only, day 5 — wait, do not ping. `eltociear#119`, `GetBindu#195`, `hermes#395`, `nix#606`: our 20.09 reminders still last, no replies. `tolkonepiu#384`: untouched since 08-29, ping no earlier than 26.09. `ai-boost#240`: still `dirty`, no merge signal — leave per the standing rule, no daily rebase. `outline#34`: merged 07:21Z (day pass recorded, nothing owed). `atlas#105`, `slopstop#633`, `nanocoder#1197`, `kilocode#13843`/`#12707`, `pr-agent#2499`, `lastlight#372`, `GitNexus#3127`, `pi-extensions#1204`, `pyrefly#4583`, `Dokploy/mcp#81`, `gbrain#5001`, `Aspire#1575`, `harness#1284`, `factory#1078`, `fullsend#297`, `stella#6461`, `cxpak#103`, `headroom#1009`: all unchanged. `deslop#173` closed 09-08. `atlas#147` (maintainer's internal issue citing our claims-gate): no action, per day pass.

**Mention sweep (`scripts/mention-sweep.sh`, both passes): 3 hits, all classified, nothing new.** `crazy54/awesome-agentic-atlas` (rejected 09-16, link-farm — stays rejected), `mrchatam/Trace` (same-named rival Go binary — competitor pointer, no contact), `yzfly/awesome-context-engineering` (our merged listing). Left out of `mentions-seen.txt` deliberately, same as previous runs — they reprint every pass and classify in one line.

**Catalogs: moratorium holds, no submissions.** Queue stays StackMap → aloth → TensorBlock; `acquisition` still unreadable (TRA-886). Competitors: no new doors this pass — the only competitor-shaped movement is `mrchatam/Trace` already on file.

**Touch budget: 0 of 5 used.** No outgoing messages — both findings are inbound or terminal.
