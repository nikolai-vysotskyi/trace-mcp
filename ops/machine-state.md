...
| tweakcc prompts written to `~/.tweakcc`, then **`npx -y tweakcc@<pinned> --apply` patches the installed Claude Code bundle** (`src/init/tweakcc.ts`) — fetching tweakcc if absent. Version pinned by `TWEAKCC_VERSION` and declared in `package.json`, so it is in the SBOM and dependabot (TRA-1133); detection is filesystem-only, so nothing is downloaded and nothing third-party runs before the tier question | `init` Max tier, **and `init --yes` unconditionally** | the tier prompt now names the exact spec it would run; `--yes` still says nothing | tweakcc's own restore | **Consent** — this is the one place we cross the line `ops/context-block-levers.md` draws |
