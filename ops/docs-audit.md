...
| --- | --- | --- | --- |
| 2026-09-09 | `docs/quality-gates.md`, `docs/analytics.md`, `docs/configuration.md` (client table), `docs/development.md`, `README.md` (quality-gates link) | `src/tools/quality/quality-gates.ts`, `src/tools/register/quality.ts`, `src/cli/check.ts`, `src/analytics/benchmark.ts`, `src/init/mcp-client.ts`, `tests/` | Four defects corrected. (1) `docs/quality-gates.md` claimed three rules run by default on unconfigured projects; in reality `check_quality_gates` returns `NO_GATES_CONFIGURED` unless `use_default_gates: true` is passed (preventing false PASS in CI), while CLI `trace check` has its own distinct default set (including debt grade and security). Documented both. (2) `README.md` claimed quality gates check "coverage thresholds" — coverage is not one of the gate signals (documented as explicitly absent in `quality-gates.md`). (3) `docs/analytics.md` claimed `benchmark_project` has 5 scenarios; it has 11 (`src/analytics/benchmark.ts`), and updated the state directory in its ASCII diagram to `~/.trace/`. (4) `docs/configuration.md` table of supported clients was missing OpenCode (`~/.config/opencode/opencode.json[c]`), added in TRA-1204. `docs/development.md` test command reference updated. |
...
- `docs/telemetry.md`, `CONTRIBUTING.md`
