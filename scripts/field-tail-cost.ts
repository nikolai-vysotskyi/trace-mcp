...
const field = new Map<string, { calls: number; meanChars: number }>();
for (const r of db
  .prepare(
  ...
  field.set(r.tool, { calls: r.n, meanChars: r.chars });
...
for (const [tool, rec] of Object.entries(store.per_tool)) {
  ...
  const f = field.get(tool);
  // No recorded response: the tool was called through a client the analytics
  // miner never saw. Counted as uncovered, never imputed — an average of the
  // priced ones would be a number about the priced ones.
  if (!f) {
    unpriced.push({ tool, calls });
    continue;
  }
  const perCall = Math.round(f.meanChars * ratio);
  const baseline = rawCostFor(tool);
  rows.push({
    tool,
    calls,
    field_calls: f.calls,
    ...
    measured_tokens: perCall * calls,
    no_baseline: NO_BASELINE_TOOLS.has(tool),
  });
}
...
const out = {
  ...
  source: '~/.trace/analytics.db + ~/.trace/savings.json, one machine (maintainer)',
  chars_to_tokens: {
    ratio: Number(ratio.toFixed(4)),
    from: 'median of the 24 harness-measured tools in docs/perf/response-tokens.json',
  ...
  coverage_pct_before: Number(
    ((100 * published.calls_weighted) / (published.calls_weighted + tailCalls)).toFixed(1),
  ),
  coverage_pct_after: Number(
    (
      (100 * (published.calls_weighted + sum(rows.map((r) => r.calls)))) /
      (published.calls_weighted + tailCalls)
    ).toFixed(1),
  ),
