...
/**
 * How far a committed `<lastmod>` may sit behind the page's commit date before
 * the guard calls it stale. It is slack for the *guard only* — the generator
 * always writes the git date (see `refresh`).
 *
 * An exact match is unreachable at commit time. A GitHub squash stamps the
 * squashed commit with the author date of the PR's LAST commit, not of the
 * commit that edited the page — so a PR whose docs edit (and `pnpm
 * docs:sitemap` run) happened on 09-03 and whose final review fixup landed
 * 09-04 gets every one of its pages dated 09-04 the moment it merges, against
 * a sitemap that can only ever say 09-03. #841 did exactly that to 18 pages at
 * once: green on the PR, red on master seconds later, blocking everyone else's
 * `test` job until someone regenerated by hand (TRA-795, TRA-800). No
 * commit-time mechanism can close that gap — the date the guard wants is
 * chosen at merge time, after the last commit exists.
 *
 * Two days covers that residue (documented size: one day) plus a timezone
 * boundary. It used to be seven, which also made the *generator* a no-op
 * inside the window: 14 of 28 pages were serving Google a date up to two days
 * behind their content, and re-running the generator changed nothing (TRA-995).
 * Splitting the generator from the guard is what lets this number stay small —
 * it now only decides how long a forgotten regenerate goes unnoticed.
 *
 * ponytail: one constant instead of teaching the guard to predict the merge
 * date. A PR that stays open longer than two days AND touches docs lands stale
 * and reddens master until someone re-runs the generator; raise the window
 * rather than reintroduce a hook.
 */
export const DRIFT_TOLERANCE_DAYS = 2;
 ...
 * tests/docs/page-dates.test.ts exists to protect — but it moves forward as
 * soon as git does. Google only re-crawls what claims to have changed, and on
 * this domain the sitemap is the only discovery channel that has ever produced
 * an indexed page (GSC URL Inspection, 2026-09-06: 13/13), so a date the
 * generator declines to advance is a re-crawl that never happens.
 */
export function refresh(committed, fresh) {
  if (!committed) return fresh;
  return fresh > committed ? fresh : committed;
}

/**
 * Whether a committed `<lastmod>` is stale enough to fail CI. Deliberately
 * laxer than `refresh`: the generator's job is a fresh date, the guard's is to
 * notice a regenerate nobody ran — without firing on merge-date residue.
 */
export function isStale(committed, fresh) {
  if (!committed) return true;
  return (Date.parse(fresh) - Date.parse(committed)) / 86_400_000 > DRIFT_TOLERANCE_DAYS;
}
