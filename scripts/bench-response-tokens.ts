 ...
 * one-sample defect `search` did. Measuring them showed the single samples were
 * not close: `get_outline` was published from `src/savings.ts`, the **most
 * expensive of fifteen files spanning a 17x spread** — 1 427 tokens against a
 * 558 basket mean. `search_text` went the other way, 1 722 published against a
 * 2 053 mean over a 1.7x spread. The two errors point opposite ways, which is
 * the argument for baskets: a single sample is noise, not a bias anyone could
 * correct for after the fact.
 *
 * Files are a size-stratified sample of this repo's own source: every non-test
 * TypeScript file under `src/` sorted by line count, fourteen taken at even
 ...
 * aggregate. The first fix for that — fifteen single lowercase subsystem words —
 * was worse than it looked, and code review caught it: that is exactly the query
 * shape where a `__module__:<filename>` pseudo-symbol wins the 10x FTS name
 * weight, so the basket was enriched for the very defect the same PR fixed. It
 * measured the response-shaping change at -39.3%; a basket of symbol names
 * measured the same change at -4.9%.
 *
 * The frame below is built to match the shape of real usage instead of anyone's
 * intuition. 1,133 recorded `search` calls in `~/.trace/analytics.db` (the
 * maintainer's own store, other repositories, not replayable here because they
 * target codebases this bench does not index) break down as **24.9% single
 * lowercase words** and 75.1% identifiers or phrases, mean 1.69 words. So this
 * basket is five filename-shaped words to fifteen symbol names — 25/75, the
 * measured mix — and neither half decides it alone.
 *
 * The symbol names are generated, not chosen: `scripts/gen-symbol-query-basket.mjs`
 * takes every Nth symbol ordered by `symbol_id`. Pseudo-symbols are named after
 * files and never after symbols, so that half cannot be aimed at the treatment.
 * The five words are the highest-volume subsystem terms from the earlier basket,
 * kept so the filename-shaped case stays represented at its real weight rather
 * than dropped because it flattered us.
 *
 * Provenance note: an earlier revision of this comment claimed the terms were
 * "the fifteen most common subsystem nouns in this repo's own directory names".
 * That was false — eleven of the fifteen are not directory names. The claim was
 * not checked before it was written.
 */
const SEARCH_BASKET = [
  // 25%: filename-shaped, the shape that exposes module-body pseudo-symbols.
  'indexer',
  'daemon',
  'telemetry',
  'session',
  'config',
  // 75%: symbol names, generated by scripts/gen-symbol-query-basket.mjs.
  'Error',
  'CoverageGap',
  'fastInference',
  'diagnoseTopology',
