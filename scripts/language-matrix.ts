...
/**
 * `updated:` is owned by `pnpm docs:sitemap` (`stampUpdated` in
 * scripts/gen-sitemap.mjs), which derives it from git history with a drift
 * tolerance so it never disagrees with the page's <lastmod>. Stamping
 * `new Date()` here instead raced that mechanism: the two ran independently
 * and rewrote the same key from two different clocks, so a same-day content
 * edit landed a front-matter date the sitemap's own damping had deliberately
 * held back a day (TRA-900). Carry the already-committed value forward
 * instead — `docs:sitemap` is what's allowed to move it.
 */
function currentUpdatedDate(): string {
  const today = new Date().toISOString().slice(0, 10);
  if (!fs.existsSync(OUT)) return today;
