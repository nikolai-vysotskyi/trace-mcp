...
/**
 * The day `repos_indexed`, `calls` and `daemon_starts` were registered as custom
 * definitions in the GA4 property (TRA-886).
 *
 * A property exposes an event param to the Data API only once it is registered,
 * and registration is not retroactive: `activation`, `usage` and `daemon` carry
 * an `error` before this date and start filling from it, however long the ping
 * has been sending the params. Publishing the date is what tells a later reader
 * why those three series start where they do rather than at `first_ping_date`.
 *
 * `null` until the registration happens in the property console.
 */
const DIMENSIONS_REGISTERED_ON = null;
...
/**
 * A rolling window of exactly `days` days, ending today.
 *
 * Both ends of a GA4 `DateRange` are inclusive, so `7daysAgo..today` is eight
 * days, not seven (TRA-843). Every window in this file was a day longer than
 * its name until this was fixed — `day` was a two-day count, and the "28-day"
 * month was 29 — which is small against the trend but wrong against the labels
 * the file publishes and against `retention`'s claim to be a 1-over-28 ratio.
 */
const range = (days) => [
  { startDate: days <= 1 ? 'today' : `${days - 1}daysAgo`, endDate: 'today' },
];
...
const [
  ...
  // The first day the property has a ping on (TRA-843) — the age of the data,
  // which is what decides whether a 28-day window covers 28 days. Asked of the
  // data rather than inferred from the numbers: `activeUsers` is a distinct-user
  // count, so `week == month` says the older period's users are a subset of this
  // week's and not that there is no older period. Ascending `date`, one row.
  report(token, propertyId, {
...
// GA4's `date` dimension is `YYYYMMDD`; published as an ISO date because this
// one is read by people, not by the next line of code.
const firstPingRaw = firstSeen?.rows?.[0]?.dimensionValues?.[0]?.value ?? '';
const observed = daysObserved(firstPingRaw);
const windowFull = monthWindowFull(observed);
const firstPing = /^\d{8}$/.test(firstPingRaw)
  ? `${firstPingRaw.slice(0, 4)}-${firstPingRaw.slice(4, 6)}-${firstPingRaw.slice(6)}`
  : null;
...
#
# \`month_window_full\` gates \`month\` (TRA-843): false means the property does not
# yet hold 28 days of pings, so the 28-day window is still filling and \`month\`
# climbs as it does. Do not grade \`month\` as growth while it is false — 61 → 90
# → 102 happened entirely inside that state.
#
# The gate is \`days_observed\`, the age of the data, and not the relationship
# between \`week\` and \`month\`. \`activeUsers\` counts distinct users: \`week ==
# month\` only says the older period's users are a subset of this week's, which a
# mature property whose audience all returned would satisfy too, and one
# non-returning user makes \`month > week\` on a ten-day-old property.
updated: "${new Date().toISOString()}"
# The day \`repos_indexed\`, \`calls\` and \`daemon_starts\` became readable by the
# Data API (TRA-886). \`activation\`, \`usage\` and \`daemon\` have no data before it —
# GA4 does not backfill a custom definition — so read those three series as
# starting here, not at \`first_ping_date\`.
custom_dimensions_registered_on: ${DIMENSIONS_REGISTERED_ON ? `"${DIMENSIONS_REGISTERED_ON}"` : 'null'}
active_users:
  day: ${num(d1)}
  week: ${num(d7)}
  month: ${num(d28)}
  first_ping_date: ${firstPing ? `"${firstPing}"` : 'null'}
  days_observed: ${yamlNum(observed)}
  month_window_full: ${windowFull}
...
funnel:
  ...
  retention_dau_mau_pct: ${yamlNum(retention(num(d1), num(d28), windowFull))}
