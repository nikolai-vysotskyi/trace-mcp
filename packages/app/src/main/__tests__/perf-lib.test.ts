...
import { fitGrowth, median, p95, parseCpuTime, round, thin } from '../../../scripts/perf-lib.mjs';
...
describe('parseCpuTime', () => {
  it('parses every ps cputime shape into seconds', () => {
    expect(parseCpuTime('0:00.00')).toBe(0);
    expect(parseCpuTime('1:30.50')).toBeCloseTo(90.5);
    expect(parseCpuTime('2:03:04.00')).toBe(2 * 3600 + 3 * 60 + 4);
    // ps switches to dd-hh:mm:ss past a day, which a naive split on ':' reads
    // as a two-field time and under-reports by orders of magnitude.
    expect(parseCpuTime('1-02:00:00.00')).toBe(26 * 3600);
  });
});
