...
import { Badge, StatusDot, type Tone } from '../lattice/ui';
...
/** Grade tone; the fill/contrast pair itself lives in the Badge primitive. */
...
// ── Checkbox with a 24×24 hit target ──────────────────────────────────────
    ...
    >
      ...
      <td className="px-3 text-center">
        {project.techDebtGrade ? (
          // The letter alone is not a label — spell the metric out for AT.
          <span role="img" aria-label={`Tech-debt grade ${project.techDebtGrade}`}>
            <Badge tone={GRADE_TONE[project.techDebtGrade]}>{project.techDebtGrade}</Badge>
          </span>
        ) : (
          <span style={{ color: 'var(--text-tertiary)' }}>—</span>
        )}
