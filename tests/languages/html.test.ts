...
function parse(source: string, filePath = 'index.html') {
...
describe('HtmlLanguagePlugin', () => {
  it('extracts script src as import edge', () => {
    const r = parse('<script src="app.js"></script>');
  ...
  it('extracts inline script blocks', () => {
    const r = parse('<script>console.log("hi")</script>');
    expect(r.symbols.some((s) => s.name === 'inline-script')).toBe(true);
  });

  it('extracts link stylesheet as import edge', () => {
    const r = parse('<link rel="stylesheet" href="style.css">');
    expect(r.edges).toContainEqual(expect.objectContaining({
      metadata: expect.objectContaining({ from: 'style.css', kind: 'stylesheet' }),
    }));
  });

  it('extracts HTML IDs', () => {
    const r = parse('<div id="app"></div><section id="main"></section>');
    expect(r.symbols.some((s) => s.name === '#app')).toBe(true);
    expect(r.symbols.some((s) => s.name === '#main')).toBe(true);
  });

  it('extracts custom elements', () => {
    const r = parse('<my-component></my-component><x-button></x-button>');
    expect(r.symbols.some((s) => s.name === '<my-component>')).toBe(true);
    expect(r.symbols.some((s) => s.name === '<x-button>')).toBe(true);
  });

  it('deduplicates custom elements', () => {
    const r = parse('<my-comp></my-comp><my-comp></my-comp>');
    const count = r.symbols.filter((s) => s.name === '<my-comp>').length;
    expect(count).toBe(1);
  });

  it('extracts meta tags', () => {
    const r = parse('<meta name="description" content="Hello world">');
  ...
  it('extracts form elements with name', () => {
    const r = parse('<input name="email" type="text"><select name="country"></select>');
    expect(r.symbols.some((s) => s.name === 'form:email')).toBe(true);
    expect(r.symbols.some((s) => s.name === 'form:country')).toBe(true);
  });

  it('extracts img src as import edge', () => {
    const r = parse('<img src="logo.png" alt="Logo">');
    expect(r.edges).toContainEqual(expect.objectContaining({
      metadata: { from: 'logo.png', kind: 'image' },
    }));
  });

  it('returns language html', () => {
    const r = parse('<div>hello</div>');
