...
describe('tailwind — detection', () => {
  ...
  it('returns false when not present', () => expect(new TailwindPlugin().detect(makeCtx({ react: '^18.0.0' }))).toBe(false));
});
...
.header { @apply bg-black text-white p-4; }`;
  ...
  it('extracts @responsive classes', () => {
    const classes = data.edges!.filter(e => e.edgeType === 'tailwind_custom_class').map(e => e.metadata.className);
  ...
  it('extracts @variants hover, focus', () => {
    const g = data.edges!.filter(e => e.edgeType === 'tailwind_variant_group');
    expect(g.some(e => e.metadata.variants.includes('hover') && e.metadata.variants.includes('focus'))).toBe(true);
  });
...
describe('tailwind — v2 config', () => {
  ...
  it('extracts darkMode=class', () => expect(data.symbols.find(s => s.name === 'tailwind:config')!.metadata!.darkMode).toBe('class'));
...
describe('tailwind — v3 config', () => {
  ...
  it('extracts content paths', () => expect(data.symbols.find(s => s.name === 'tailwind:content')!.metadata!.paths).toContain('./resources/**/*.blade.php'));
  it('extracts safelist', () => expect(data.symbols.find(s => s.name === 'tailwind:safelist')!.metadata!.patterns).toContain('lg:text-4xl'));
  it('extracts important as selector', () => expect(data.symbols.find(s => s.name === 'tailwind:config')!.metadata!.important).toBe('#app'));
  it('extracts darkMode=selector', () => expect(data.symbols.find(s => s.name === 'tailwind:config')!.metadata!.darkMode).toBe('selector'));
  ...
  it('extracts presets', () => expect(data.symbols.find(s => s.name === 'tailwind:config')!.metadata!.presets).toContain('./brand-preset'));
  ...
  it('extracts require() plugins', () => {
    const edges = data.edges!.filter(e => e.edgeType === 'tailwind_uses_plugin' && !e.metadata.isInline);
    expect(edges.map(e => e.metadata.pluginName)).toContain('@tailwindcss/forms');
...
describe('tailwind — v4 CSS', () => {
  ...
  it('extracts @config reference', () => expect(data.symbols.find(s => s.name === 'tailwind:v4:config-ref')!.metadata!.configPath).toBe('./tailwind.config.ts'));
  ...
  it('detects @theme inline modifier', () => {
    const s = data.symbols.find(s => s.name.includes('theme:color:inline') || (s.name.startsWith('tailwind:v4:theme:') && s.metadata!.isInline))!;
  ...
  it('detects @layer theme', () => expect(data.symbols.find(s => s.name === 'tailwind:v4:layer-theme')).toBeDefined());
  ...
  it('extracts @layer custom classes with correct layer', () => {
    ...
    expect(classes.find(e => e.metadata.className === 'btn-primary')?.metadata.layer).toBe('components');
    expect(classes.find(e => e.metadata.className === 'content-auto')?.metadata.layer).toBe('utilities');
  });
...
describe('tailwind — edge cases', () => {
  ...
  it('returns empty result for unrelated TS file', () => {
    const data = extract(p, 'service.ts', 'export class UserService { getUser() {} }', 'typescript');
