...
`,
    ...
    expect(
      r.edges!.some((e) => e.metadata?.library === 'httpx' && e.metadata?.method === 'GET'),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      r.edges!.some((e) => e.metadata?.library === 'aiohttp' && e.metadata?.method === 'GET'),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      r.edges!.some(
        (e) => e.metadata?.template === 'page.html' && e.metadata?.via === 'get_template',
      ),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      r.edges!.some(
        (e) => e.metadata?.template === 'users/list.html' && e.metadata?.via === 'render_template',
      ),
    ).toBe(true);
  });
});
...
`,
    ...
    expect(
      r.edges!.some((e) => e.edgeType === 'ml_model_class' && e.metadata?.className === 'MyNet'),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      loads.some(
        (e) => e.metadata?.model === 'bert-base-uncased' && e.metadata?.loader === 'AutoModel',
      ),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      r.edges!.some(
        (e) => e.metadata?.kind === 'pipeline' && e.metadata?.task === 'sentiment-analysis',
      ),
    ).toBe(true);
  });
...
`,
    ...
    const loads = r.edges!.filter(
      (e) => e.edgeType === 'ml_model_load' && e.metadata?.kind === 'sentence_transformer',
    );
    expect(loads.length).toBe(2);
    expect(
      loads.some(
        (e) =>
          e.metadata?.loader === 'SentenceTransformer' && e.metadata?.model === 'all-MiniLM-L6-v2',
      ),
    ).toBe(true);
...
`,
    ...
    const submods = r
      .edges!.filter((e) => e.metadata?.library === 'scipy')
      .map((e) => e.metadata?.submodule);
...
`,
    ...
    const submods = r
      .edges!.filter((e) => e.metadata?.library === 'scikit-image')
      .map((e) => e.metadata?.submodule);
...
`,
    ...
    expect(
      r.edges!.some(
        (e) =>
          e.metadata?.library === 'pillow' &&
          e.metadata?.direction === 'read' &&
          e.metadata?.target === 'photo.jpg',
      ),
    ).toBe(true);
    expect(
      r.edges!.some((e) => e.metadata?.library === 'pillow' && e.metadata?.direction === 'write'),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      r.edges!.some((e) => e.metadata?.library === 'opencv' && e.metadata?.direction === 'read'),
    ).toBe(true);
    expect(
      r.edges!.some((e) => e.metadata?.library === 'opencv' && e.metadata?.direction === 'write'),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      r.edges!.some((e) => e.metadata?.library === 'imageio' && e.metadata?.direction === 'read'),
    ).toBe(true);
    expect(
      r.edges!.some((e) => e.metadata?.library === 'imageio' && e.metadata?.direction === 'write'),
    ).toBe(true);
  });
});
...
`,
    ...
    const apis = r
      .edges!.filter((e) => e.edgeType === 'async_primitive')
      .map((e) => e.metadata?.api);
...
`,
    ...
    expect(
      r.edges!.some((e) => e.edgeType === 'attrs_class' && e.metadata?.className === 'Point'),
    ).toBe(true);
  });
...
`,
    ...
    expect(
      r.edges!.some((e) => e.edgeType === 'attrs_class' && e.metadata?.className === 'User'),
    ).toBe(true);
  });
});
